...
const createRestaurantItemTemplate = (restaurant) => `
  <article class="restoran-item">
    <a href="/#/detail/${restaurant.id}">
      <img class="restoran-item__thumbnail" src="${
        CONFIG.BASE_IMAGE_URL + restaurant.pictureId
      }" alt="${restaurant.name}">
      <div class="restoran-item__content">
      <h3 class="restoran-item__name">${restaurant.name}</h3>
        <p class="restoran-item__city">Available at ${restaurant.city}  
        <span class="restoran-item__rating" aria-label="rating restoran ${
          restaurant.rating
        }"> <br> <span>Rating : </span><i class="fa fa-star"></i>${
  restaurant.rating
}</span>
        </p>
        <p class="card_item__description" >${restaurant.description}</p>
        <button class="restoran_item__btn_read"> Selengkapnya </button>      
        </div>
    </a>
  </article>
`;
  ...
  <div class="restaurant-detail__wrapper">
    <div class="restaurant-detail__info">
      ...
      <p class="restaurant-detail__about">
        <span class="restaurant-detail__rating"><i class="fa fa-star"></i>${
          restaurant.rating
        }</span> 
      ...
      <p class="restoran-detail__location font-secondary">${
        restaurant.address
      }, ${restaurant.city}</p>
      <img class="restaurant-detail__thumbnail" src="${
        CONFIG.BASE_IMAGE_URL + restaurant.pictureId
      }" alt="${restaurant.name}" />
